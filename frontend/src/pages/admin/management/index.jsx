...
function AdminManagementPage() {
    ...
    const refreshDirectories = useCallback(async () => {
        try {
            ...
            setDirectories(contacts.map((c, i) => ({
                id: c.id || i,
                caseId: `#${(c.category || "dir").toUpperCase().slice(0, 3)}-${String(c.id || i).padStart(3, "0")}`,
                name: c.name,
                phone: c.phone || c.phone_alt || "NOT ASSIGNED",
    ...
    return (
        <AdminLayout activeTab="management">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white rounded-2xl p-4 shadow-[0px_8px_24px_rgba(149,157,165,0.1)] border border-slate-100">
                            ...
                            <div className="space-y-2">
                                <button 
                                    onClick={() => { setEditContact(null); setIsAddDirOpen(true); }}
                        ...
                        <div className="space-y-4">
                            {[...directories]
                                ...
                                .filter(dir => {
                                    ...
                                    const matchFilter = !dirFilter ||
                                        (dir.type || "").toLowerCase() === dirFilter.toLowerCase()
                                ...
                                .map(dir => (
                                    <div key={dir.id} className="bg-white rounded-2xl p-4 shadow-[0px_8px_24px_rgba(149,157,165,0.1)] border border-slate-100 flex flex-col relative">
                                        ...
                                        <div className="flex justify-end gap-2 mt-auto">
                                            <button 
                                                onClick={() => { setMapContact(dir.raw || dir); setIsMapOpen(true); }}
                                                className="p-2 bg-slate-100 text-slate-500 rounded-lg hover:bg-slate-200 transition-colors"
                                            >
                                                <Map size={16} />
                                            </button>
                                            <button 
                                                onClick={() => { setEditContact(dir.raw || dir); setIsAddDirOpen(true); }}
            ...
            <DirectoryAddDialog
                isOpen={isAddDirOpen}
                onClose={() => { setIsAddDirOpen(false); setEditContact(null); }}
            ...
            <DirectoryMapDialog
                isOpen={isMapOpen}
                onClose={() => { setIsMapOpen(false); setMapContact(null); }}
                contact={mapContact}
            />
        </AdminLayout>
    )
