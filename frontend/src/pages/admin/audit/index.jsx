...
function AdminAuditPage() {
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const response = await fetch("http://localhost:5000/api/reports?per_page=100", { headers: getAuthHeaders() })
            if (response.ok) {
        ...
        reports.forEach((report) => {
            if (report.status === "approved_awareness" && report.review_date) {
                entries.push({
                    ...
                    notes: report.review_notes || "", timestamp: report.review_date,
            ...
            if (report.status === "dismissed" && report.review_date) {
                entries.push({
                    ...
                    refCode: report.reference_code || `SF-${report.id}`, title: report.title, category: report.category,
                    reviewedBy: report.reviewed_by ? `Admin-${String(report.reviewed_by).padStart(2, "0")}` : "System",
                    notes: report.review_notes || "Dismissed as spam/duplicate", timestamp: report.review_date,
            ...
            <div className="w-full max-w-sm px-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="px-3 py-1.5 bg-indigo-50 rounded-full border border-indigo-200 flex items-center gap-1.5">
                        ...
                        <span className="text-indigo-600 text-[11px] font-semibold font-['DM_Sans']">{auditStats.total} Events</span>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full max-w-sm px-4 mt-5">
                <div className="w-full bg-[#3b53cc] rounded-xl p-5">
                    ...
                    <p className="text-blue-100 text-[11px] font-normal font-['DM_Sans'] leading-relaxed">
                        All administrative actions are logged and retained for 180 days per SafeMap data governance policy. Audit records are immutable and tamper-proof.
                    </p>
                </div>
            </div>
        </AdminLayout>
